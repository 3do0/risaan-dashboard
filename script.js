document.addEventListener('DOMContentLoaded', () => {
  ...
  const updateDocumentLanguage = () => {
    ...
    const homeTitle =
      translations[currentLanguage]?.meta?.homeTitle ||
      translations[currentLanguage]?.meta?.title;
    if (homeTitle) {
  ...
  systemCards.forEach(card => {
    ...
    card.addEventListener('click', function(e) {
      ...
      if (systemType && SUPPORTED_SYSTEM_KEYS.includes(systemType)) {
        localStorage.setItem('selectedSystemKey', systemType);
      }
      const baseHref = this.getAttribute('data-base-href') || this.getAttribute('href');
      const destination = buildUrlWithLang(baseHref, currentLanguage);
      if (destination) {
