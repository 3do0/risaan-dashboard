document.addEventListener("DOMContentLoaded", () => {
  ...
  const updateLangAwareLinks = () => {
    document.querySelectorAll("[data-keep-lang]").forEach((link) => {
      const baseHref =
        link.getAttribute("data-base-href") || link.getAttribute("href");
  ...
  const updateDocumentLanguage = () => {
    ...
    document.documentElement.dir =
      translations[currentLanguage]?.direction || "rtl";
  ...
  const updateSystemLabels = () => {
    ...
    const activeName =
      systemNames[currentSystemKey] || systemNames[SUPPORTED_SYSTEM_KEYS[0]];
    ...
    if (systemTitleElement && activeName) {
      const suffix =
        translations[currentLanguage]?.pageInfo?.dashboardSuffix || "";
      systemTitleElement.textContent = suffix
        ? `${activeName} – ${suffix}`
        : activeName;
    }
  ...
  const navLinks = document.querySelectorAll(
    ".nav-link:not(.nav-link-group .nav-link)"
  );
  ...
  if (navUser && userDropdown) {
    ...
    const userDropdownItems = userDropdown.querySelectorAll(
      ".user-dropdown-item"
    );
  ...
  if (systemSwitcherBtn && systemDropdown) {
    ...
    document.addEventListener("click", function (e) {
      if (
        !systemSwitcherBtn.contains(e.target) &&
        !systemDropdown.contains(e.target)
      ) {
        systemDropdown.classList.remove("active");
      }
    });

    const systemDropdownItems = systemDropdown.querySelectorAll(
